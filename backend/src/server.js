...
export async function build({ dbPath = DB, logger = false } = {}) {
  ...
  // Walk up to the repo root, then into mobile/www — works whether you run
  // `pnpm dev` from the backend dir or the repo root.
  const wwwRoot = join(__dirname, '..', '..', 'mobile', 'www');
